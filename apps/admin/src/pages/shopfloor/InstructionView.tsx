  ...
  return (
    <ol className="space-y-3">
      {instruction.steps.map((step, i) => (
        <Step key={step.id} step={step} index={i} checked={checked} onToggle={onToggle} large={large} />
      ))}
    </ol>
  )
}
...
function Step({
  ...
  return (
    <li className={cn('rounded-2xl p-4', safety ? 'bg-accent-soft text-accent-strong' : 'bg-surface-2')}>
      <div className="gap-3 flex items-start">
        <span
          className={cn(
            'size-9 rounded-xl text-xs font-bold flex shrink-0 items-center justify-center tabular-nums',
            safety ? 'text-white bg-accent' : 'bg-card shadow-card',
          )}
        >
          {index + 1}
        </span>
        <div className="min-w-0 flex-1">
          <div className="gap-2 flex flex-wrap items-center">
            ...
            <h4 className={cn('font-semibold', large ? 'text-lg' : 'text-sm')}>{step.title}</h4>
          ...
          {step.kind === 'setup' ? (
            <KeyValue
              bare
              labelWidth="md"
              className="mt-2 rounded-xl px-3 bg-card"
                ...
                return (
                  <li key={key}>
                    <button
                      ...
                      className={cn(
                        'min-h-12 gap-3 rounded-xl px-3 py-2 flex w-full items-center bg-card text-left transition-colors hover:bg-surface focus-visible:ring-2 focus-visible:ring-accent/40 focus-visible:outline-none',
                        large ? 'text-base' : 'text-sm',
                      )}
                    >
                      <span
                        className={cn(
                          'size-6 [&_svg]:size-3.5 flex shrink-0 items-center justify-center rounded-full border-2',
                          done ? 'text-white border-success bg-success' : 'border-border',
                        )}
                      >
          ...
          {step.attachment && (
            <span className="mt-3 h-9 gap-2 px-3 text-xs font-semibold inline-flex items-center rounded-full bg-card shadow-card">
